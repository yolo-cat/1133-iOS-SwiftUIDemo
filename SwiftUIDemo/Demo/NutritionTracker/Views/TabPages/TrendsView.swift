...
struct TrendsView: View {
                ...
                ForEach(TimeRange.allCases, id: \.self) { range in
                    Button(action: {
                        ...
                        Text(range.displayName)
                            .font(.subheadline)
                            .fontWeight(.medium)
                            .foregroundColor(selectedTimeRange == range ? .white : .primaryBlue)
                            .padding(.horizontal, 16)
                            .padding(.vertical, 8)
                            .background(selectedTimeRange == range ? .primaryBlue : .primaryBlue.opacity(0.1))
                            .cornerRadius(20)
                    }
                }
    ...
    private func achievementBadge(_ title: String, icon: String, isUnlocked: Bool) -> some View {
        ...
        .frame(maxWidth: .infinity)
        .padding(.vertical, 12)
        .background(isUnlocked ? .primaryBlue.opacity(0.1) : .gray.opacity(0.1))
        .cornerRadius(12)
