                    ...
                    Button(action: {
                        ...
                        VStack(spacing: 8) {
                            Image(systemName: mealType.icon)
                                .font(.title2)
                                .foregroundColor(selectedMealType == mealType ? .white : .primaryBlue)
                        ...
                        .padding(.vertical, 16)
                        .background(selectedMealType == mealType ? .primaryBlue : .primaryBlue.opacity(0.1))
                        .cornerRadius(12)
                    }
                }
            }
    ...
    private func foodItemCard(_ food: MealItem) -> some View {
        Button(action: {
            ...
            .background(selectedFoods.contains(where: { $0.id == food.id }) ? 
                       .primaryBlue.opacity(0.1) : .white)
            .cornerRadius(12)
            .overlay(
                RoundedRectangle(cornerRadius: 12)
                    .stroke(selectedFoods.contains(where: { $0.id == food.id }) ? 
                           .primaryBlue : .clear, lineWidth: 2)
            )
            .shadow(color: .black.opacity(0.05), radius: 2, x: 0, y: 1)
        }
    }
